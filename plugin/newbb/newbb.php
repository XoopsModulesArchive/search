...
function b_search_newbb($queryarray, $andor, $limit, $offset, $userid)
{
    ...
    if (1 == $showcontext) {
        $sql = 'SELECT p.post_id,p.topic_id,p.forum_id,p.post_time,p.uid,p.subject,p.nohtml,p.nosmiley,t.post_text FROM '
               . $xoopsDB->prefix('bb_posts')
               . ' p LEFT JOIN '
               . $xoopsDB->prefix('bb_posts_text')
               . ' t ON t.post_id=p.post_id LEFT JOIN '
               . $xoopsDB->prefix('bb_forums')
               . ' f ON f.forum_id=p.forum_id WHERE f.forum_type=0';
    } else {
        $sql = 'SELECT p.post_id,p.topic_id,p.forum_id,p.post_time,p.uid,p.subject,p.nohtml,p.nosmiley FROM '
               . $xoopsDB->prefix('bb_posts')
               . ' p LEFT JOIN '
               . $xoopsDB->prefix('bb_posts_text')
               . ' t ON t.post_id=p.post_id LEFT JOIN '
