...
switch ($op) {
    ...
    case 'edit':
        ...
        while (list($mid, $name) = each($mod_arr)) {
            $check = in_array($mid, $myrow, true) ? ' checked' : '';

            $exval = in_array($mid, $myrow, true) ? 1 : 0;

            $color = in_array($mid, $myrow, true) ? 'red' : 'black';
        ...
        break;
    case 'updata':
        $dis_mids = $_REQUEST['dis_mids'] ?? [];
        $old_mids = $_REQUEST['old_mids'] ?? [];
        $db = XoopsDatabaseFactory::getDatabaseConnection();
